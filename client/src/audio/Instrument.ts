...
export abstract class Instrument {
  ...
  generate(note: string, octave: number, duration: number): Blob {
    ...
    const noteValue = AudioSynth.noteMap.get(note);
    if (noteValue === undefined) {
      throw new Error(`undefined note:${note}`);
    }
    const frequency = noteValue * Math.pow(2, octave - 4);
    const sampleRate = this.synth.sampleRate;
    const volume = this.synth._volume;
